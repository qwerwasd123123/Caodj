...
def base_info(request):
    ...
    # raw_list = models.UserInfo.objects.filter(nid=nid)
...
def tag(request):
    ...
    if request.method=="GET":
        row_list = models.Tag.objects.all().order_by('-nid')[page_obj.start:page_obj.end]
...
def category(request):
    ...
    :return:
    """
    data_count = models.Category.objects.all().count()
    page_obj = Pagination(request.GET.get('p'), data_count)
    base_url = '/backend/category.html'
    page_str = page_obj.page_str(base_url)
    if request.method=="GET":
        row_list = models.Category.objects.all().order_by('-nid')[page_obj.start:page_obj.end]
        return render(request, 'backend_category.html', locals())
    elif request.method=="POST":
        nickname = request.POST.get('nickname')
        blog_id = request.session['user_info']['blog__nid']
        obj = models.Category(title=nickname,blog_id=blog_id)
        obj.save()
        row_list = models.Category.objects.all().order_by('-nid')[page_obj.start:page_obj.end]
        return render(request,'backend_category.html',locals())
@check_login
